...
def getDailyNews():
    sources = getSources()
    key = 'acf654c1f81d4e00b3938c47525848df'
    ...
    try:
        aux = pd.read_csv('./data-model/news.csv')
        aux = aux.append(news)
        aux = aux.drop_duplicates('url')
        aux.reset_index(inplace=True, drop=True)
        aux.to_csv('./data-model/news.csv', encoding='utf-8', index=False)
    except:
        news.to_csv('./data-model/news.csv', index=False, encoding='utf-8')
